...
export class CreatePageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = new FormGroup({
      ...
      questions: new FormArray(
        [
          this.fb.group({
            qText: this.fb.control([], Validators.required),//this.fb.control('', Validators.required),
            rightAnswerIdx: this.fb.control([], Validators.required),//this.fb.control('', Validators.required),
  ...
  /*isRightAnswerSelected(questionIndex: number, answerIndex: number): boolean {
    const questionForm = this.questions.controls[questionIndex];
    const rightAnswerIdx = questionForm.value.rightAnswerIdx;
    return Array.isArray(rightAnswerIdx) ? rightAnswerIdx.includes(answerIndex) : rightAnswerIdx === answerIndex;
  }
  isRightAnswerSelected(i: number): boolean {
    const questionForm = this.questions.controls[i];
    const rightAnswerIdx = questionForm.value.rightAnswerIdx;
    return rightAnswerIdx !== null && rightAnswerIdx.length > 0;
  } */
  ...
  isAnyRightAnswerSelected(i: number): boolean {
    const answersForm = this.getAnswers(i).controls;
    for (let j = 0; j < answersForm.length; j++) {
      const answerForm = answersForm[j];
      if (answerForm.value && answerForm.value === true) {
        return true;
      }
    }
    return false;
